...
FadeManager::FadeManager(sf::Time duration) : m_duration(duration) {}

int FadeManager::getCurrentValue() {
    return m_clock.getElapsedTime().asMilliseconds() * (m_target_val - m_start_val) /
               m_duration.asMilliseconds() +
           m_start_val;
}
