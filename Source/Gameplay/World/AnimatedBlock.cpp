...
AnimatedBlock::AnimatedBlock(short id, sf::Vertex& quad, Application& app) : m_p_quad(&quad)
{
	...
	m_block_size = app.getSettings()["block_size"];
	m_p_quad = &quad;
...
{		
	if (duration_cast<milliseconds>(steady_clock::now() - m_start) >= m_cur_frame->duration)
	{
		...
		m_p_quad[1].texCoords = sf::Vector2f(m_cur_frame->uv.x + m_block_size, m_cur_frame->uv.y);
		m_p_quad[2].texCoords = sf::Vector2f(m_cur_frame->uv.x + m_block_size, m_cur_frame->uv.y + m_block_size);
		m_p_quad[3].texCoords = sf::Vector2f(m_cur_frame->uv.x, m_cur_frame->uv.y + m_block_size);
	}
}