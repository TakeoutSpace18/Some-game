...
void TilesetManager::load(short tileset_id, Textures texture_place) {
    ...
    std::ifstream input("data/tilesets/tileset_" + std::to_string(tileset_id) + ".json");
    if (input.is_open()) {
        json json_file;
        input >> json_file;
        input.close();
        ...
        _application->loadTexture(texture_place,
                                  "data" + json_file["image"].get<std::string>().erase(0, 2));
        ...
        for (auto& tile : json_file["tiles"]) {
            ...
            if (!tile["animation"].is_null()) {
                ...
                _application->loadAnimation(animation_name, tile["animation"], *this);
            }
            ...
            if (!tile["objectgroup"]["objects"].is_null()) {
                ...
                for (auto& object : tile["objectgroup"]["objects"]) {
                    collisions.emplace_back(sf::IntRect(
                        round(object["x"].get<float>()), round(object["y"].get<float>()),
                        round(object["width"].get<float>()), round(object["height"].get<float>())));
                }
